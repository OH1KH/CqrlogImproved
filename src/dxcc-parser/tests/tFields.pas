...
var
  ...
  Diff: string;
begin
  { CqrlogImproved's older bundled ctyfiles still contain multi-window marks
    (e.g. the VQ9 lines in CountryDel.tab) which both DXCC engines expand but
    the deliberately naive reference parser does not, so the entry counts
    differ.  Upstream cqrlog's refreshed data normalised those lines.  Delete
    this Ignore once ctyfiles are synced with upstream. }
  Ignore('older ctyfiles carry multi-window marks that uRefParser does not expand');
